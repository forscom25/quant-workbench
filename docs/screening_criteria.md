...
**현금흐름 구제 (2026-09-08 도입)**: 판관비 급증·GPM 하락으로 컷오프 미달 판정된 종목이라도, ① 실제 매출성장이 있고(`sales_growth_yoy > cash_flow_rescue_min_sales_growth`, 기본 0%) ② TTM 영업활동현금흐름(OCF)이 건실하면(`OCF > 0` 이고 `OCF ≥ 당기순이익`) `na_reasons`에 `CASH_FLOW_QUALITY_RESCUE` 태그를 남기고 구제한다. 배경: 확정 수주잔고 기반으로 설비·인력에 선제 투자하는 업종(예: 방산)은 매출 대비 비용이 일시적으로 빠르게 늘어 이 단계의 하드 컷에 걸리기 쉬운데, 그 지출이 실제 현금 유입으로 뒷받침되는지(진짜 성장 vs 순수 테마성 지출)로 구분한다. 매출이 늘지 않는 경우(`NOGROWTH` 유형)는 현금흐름이 좋아도 구제 대상이 아니다 — 자세한 배경은 decisions_log.md 2026-09-08 항목 참고.
...
- [ ] 1단계 통과 비율 확정 → 2026-09-08에 0.4→0.5로 1차 완화(경계선 종목 구제 여부 검증 목적, 재검증 대기 중). 극단적 모멘텀 섹터(예: 방산)는 이 정도 완화로는 구제 안 될 것으로 예상 — 위 "알려진 한계" 참고
