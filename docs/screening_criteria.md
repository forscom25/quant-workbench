...
- [ ] 위에서 유일하게 유망하다고 확인된 `[이익질주의]` 태그를 Stage5의 실제 배제 게이트로 전환해 27개 분기 전체 A/B 백테스트로 최종 검증 (2026-09-17/18 분석은 "flagged 그룹이 실제로 더 나쁜지"만 저비용으로 확인한 것이라, 실제 포트폴리오 구성·리밸런싱까지 반영한 전체 백테스트로 재검증 필요)
- [ ] **(데스크탑에서 진행 중 — 2026-09-20 기준 3·4단계 완료, 5단계는 KRX 접속 제한으로 재개 대기. 범위는 DART 분기 데이터가 2016년부터라 2014~2018 → 2016~2018로 축소, 아래 "진행 상황" 참고)** 2014~2018 구간 out-of-sample 검증 — 2019~2025로 도출한 결론들(annual/avg_4q/per_weight 전부 열세, 5개 warning_tags 중 4개는 게이트 부적합)이 이 특정 시장 국면에 과적합된 건 아닌지 확인. **별도 DART_API_KEY 발급 권장**(같은 키를 노트북과 동시에 쓰면 일일 호출 한도를 공유하게 됨 — 2026-09-11 논의 참고). 절차:
  ...
  - **진행 상황 (2026-09-20, 상세는 [decisions_log.md](./decisions_log.md) 2026-09-20 항목)**:
    ...
    - 5단계(신호 분석 3종)는 KRX가 IP를 1일간 제한(2026-09-20 14시 무렵)해 미완 — Stage1의 유효 12개 구간(2016~2018)만 확보: 두 신호 모두 유의성 없음(2019~2025와 같은 방향). **재개 시 순차 실행**(Stage1 → Stage4 → 게이트, 병렬 금지), 2016~2025 전체를 한 번만 돌리고 `analysis/signal_period_breakdown.py`로 2016~2018(OOS)/2019~2021/2022~2025/전체를 재집계. 스크립트에 연속 3분기 실패 시 중단·분기별 중간 저장 안전장치를 추가함(`backtest/analysis_guard.py`, 실제 KRX 동작은 미검증).
