...
- **기본값은 여전히 `"ttm"`** — `"annual"`이 실제로 더 나은지는 아직 백테스트 A/B 검증 전(TODO).
...
- [ ] 위에서 구현한 `profitability_basis: "annual"`이 `"ttm"`(기존 기본값) 대비 실제로 더 나은지 백테스트 A/B 검증 — `annual_level_weight`/`annual_trend_weight`(현재 0.5/0.5)도 감으로 정한 초기값이라 함께 튜닝 대상. (아래 DART 타임아웃 버그가 선행 블로커였으나 해소되어 재시도 가능한 상태 — 실행 자체는 DART 호출량 때문에 사용자 확인 후 진행)
