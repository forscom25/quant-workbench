...
9. **"정교화"는 기본적으로 의심한다 (2026-09-17 확립)**: `profitability_basis: "annual"`, `ttm_denominator: "avg_4q"`, Stage4 `per_weight` 세 건 전부 "기존 설계에 차원/정교함을 추가"하는 시도였고 셋 다 27분기 백테스트에서 성과를 악화시켰다. Stage1(z_return/z_volume)과 Stage4(pbr_inv/bps_growth/per_inv) 양쪽에서 팩터별 IC를 직접 측정해보니 개별 팩터의 선형 예측력은 대부분 약하거나 유의하지 않았고, 그럼에도 5단계 순차 필터링 조합 자체는 검증된 성과를 냈다 — 즉 이 시스템은 "강한 개별 신호들의 정교한 조합"이 아니라 "약한 신호들의 순차 필터 앙상블"에 가깝다. 따라서 새 팩터·가중치 세분화·비선형 결합 등을 제안할 때는 먼저 `backtest/stage1_signal_analysis.py`/`stage4_signal_analysis.py`류의 저비용 IC 분석으로 개별 신호의 예측력부터 확인하고, 그 다음에만 27분기 전체 백테스트에 투자하는 순서를 기본으로 삼는다. **(2026-09-21 단서)**: "약한 신호들의 앙상블이 성과를 낸다"는 이 설명은 검증된 결론이 아니라 가설이다 — 파이프라인 전체의 분기 초과수익이 통계적으로 0과 구분되지 않고(2019~2025 -0.98%p t=-0.68, 2016~2018 -0.41%p t=-0.13) 낮은 MDD는 낮은 베타(0.66)와 구분되지 않는다. 또한 개별 팩터 IC의 부호가 국면마다 뒤집히는 사례(Stage4 `per_inv_z`: 2016~2018 +0.088 vs 2019~2025 -0.051)가 있어 2019~2025 기반의 IC 결론은 그 국면에 한정될 수 있다. 직접 검증 TODO 참고.
...
- [x] 1단계 z_return, z_volume 가중치(w1, w2) 초기값 0.5/0.5 → forward-return 상관관계로 백테스트 튜닝 — 2026-09-17, `backtest/stage1_signal_analysis.py`(신규, DART 없이 pykrx만 사용) 신설해 27개 분기·3335개 분기x섹터 표본으로 IC(정보계수) 분석. 결론: `return_z_score`·`volume_z_score` 둘 다 forward 섹터 수익률과 통계적으로 유의한 상관관계 없음(풀링 Spearman·분기평균 Fama-MacBeth·분위 스프레드 세 방법 모두 |t|<1, 부호도 방법론 따라 뒤집힘) — 이 데이터로 가중치를 0.5/0.5에서 바꿀 근거 없음, 기본값 유지 확정. Stage1은 강한 알파 신호가 아니라 "넓은 1차 필터 + DART 호출량 절감용 계산비용 관문" 역할로 rough하게 동작해도 무방하다는 결론(전체 파이프라인은 이미 27분기 백테스트로 누적 54.3%·양의 Sharpe 검증됨 — 약한 개별 신호들이 후속 단계와 결합되어 검증된 성과를 내는 구조) 
...
- [ ] (우선순위 하향, 2026-09-17) Stage1의 섹터 모멘텀 하드컷을 penalty 항(음의 가중치를 가진 점수)으로 전환할지 검토 — 개별 기업 펀더멘털이 뛰어나면 섹터가 과열이어도 만회할 수 있게 하는 방향. 다만 Stage1은 DART 호출량을 줄이는 계산비용 관문 역할도 겸하고 있어(현재는 후보군이 좁아질수록 이후 단계 계산량도 줄어드는 구조), 전 요인 통합 스코어링으로 가면 그 절약 효과가 사라져 DART 일일 한도 문제가 재발할 위험 큼 — 비용 관문(유동성/시총 등 모멘텀과 무관한 기준)과 모멘텀 신호를 분리하는 절충안(2026-09-08 논의에서의 "C안")부터 검토 예정. **2026-09-17 저비용 사전검증(`debugging/stage1_bypass_diagnostic.py`) 결과**: 방산 5종목x3개 분기(2024-12~2025-06, 언더퍼폼 격차가 가장 컸던 시기) 중 Stage1을 우회시켜 실제 통과 후보군(344~395개) 안에서 경쟁시켜본 결과 **15건 중 2건(13%)만 최종 통과** — 나머지는 대부분 Stage4(밸류에이션)에서 걸러짐(이미 급등한 뒤라 PBR이 비싸져서 정상적으로 밸류에이션 필터에 걸림). C안의 기대 효과가 애초 가설보다 작아 보여 우선순위 하향, 구현 비용(DART 예산 재폭발 위험 등)은 그대로라 지금 당장 착수하지 않기로 함
...
- [x] Stage4 `per` 필드를 composite score에 실제로 반영할지 여부 결정 — 2026-09-17, `per_weight` 파라미터 신설(기본 0.0=미반영, 기존 동작과 동일) 후 PBR 0.1/PER 0.1/BPS 0.8로 A/B 검증. 결과: 누적수익률(22.4% vs 54.3%)·MDD(-29.7% vs -25.1%)·Sharpe(0.15 vs 0.32)·승률(29.6% vs 37.0%) 전부 열세, 평균 통과 종목 수도 증가(12.0→16.4개) — annual·avg_4q와 동일한 패턴(스코어에 차원을 추가할수록 컷오프가 느슨해지고 성과는 악화). `per_weight: 0.0` 기본값 최종 확정, 코드는 보존
...
- [x] `global.ttm_denominator` 기본값을 `latest_snapshot` → `avg_4q`로 바꿀지 백테스트로 비교 검증 — 2026-09-14, 27개 분기 전체 실행 결과 `avg_4q`가 누적수익률(29.9% vs 54.3%)·MDD(-26.4% vs -25.1%)·Sharpe(0.19 vs 0.32)·승률(29.6% vs 37.0%) 전부에서 열세로 확인되어 `latest_snapshot` 기본값 유지 최종 확정. `profitability_basis: "annual"`(2026-09-12) 검증과 같은 방향의 결론 — 분모를 여러 분기로 스무딩하면 급격한 자본 변동(감액손실 등) 신호가 완화되어 위험 종목을 더 늦게 걸러내는 비용이, 의도했던 "외란 완화" 효과보다 큼. 코드는 보존(향후 재시도 가능)
- [x] 기존 `warning_tags`(판정에 미반영, 태그만 붙는 5종)를 실제 배제 게이트로 전환할 가치가 있는지 저비용 검증 — 2026-09-17/18, `backtest/warning_tag_gate_analysis.py` 신설. 각 태그가 실제 계산되는 stage의 입력 후보군 전체(통과/탈락 무관)에서 flagged vs unflagged forward return을 비교. 결과: 5개 중 **4개는 게이트 전환 시 오히려 해가 됨** — `is_cost_cutting_warning`은 flagged 그룹이 통계적으로 유의하게 더 좋은 성과(+2.00%p, p=0.031, 가설과 정반대 방향), `is_pbr_value_trap`은 무의미(p=0.87), `[ICR미달]`·`[과다부채]`도 방향이 뒤집혀 있음(비유의). **유일한 후보는 `[이익질주의]`**(OCF<순이익)로 flagged 그룹이 -3.36%p 더 나쁨(p=0.068, 근접 유의, 방향은 가설과 일치)
- [ ] 위에서 유일하게 유망하다고 확인된 `[이익질주의]` 태그를 Stage5의 실제 배제 게이트로 전환해 27개 분기 전체 A/B 백테스트로 최종 검증 (2026-09-17/18 분석은 "flagged 그룹이 실제로 더 나쁜지"만 저비용으로 확인한 것이라, 실제 포트폴리오 구성·리밸런싱까지 반영한 전체 백테스트로 재검증 필요) — **2026-09-21 우선순위 하향**: 2016~2018 out-of-sample에서 `[이익질주의]` 효과가 사실상 0(-0.08%p)이고, 2016~2025 전체로 넓히면 -2.59%p(p=0.11)로 유의성이 사라짐(효과가 2022~2025에 몰려 있음) — A/B 백테스트에 착수할 근거가 약해짐
- [x] **(데스크탑에서 완료 — 2026-09-21. 범위는 DART 분기 데이터가 2016년부터라 2014~2018 → 2016~2018로 축소, 결과는 아래 "진행 상황"과 decisions_log.md 2026-09-20~21 항목 참고)** 2014~2018 구간 out-of-sample 검증 — 2019~2025로 도출한 결론들(annual/avg_4q/per_weight 전부 열세, 5개 warning_tags 중 4개는 게이트 부적합)이 이 특정 시장 국면에 과적합된 건 아닌지 확인. **별도 DART_API_KEY 발급 권장**(같은 키를 노트북과 동시에 쓰면 일일 호출 한도를 공유하게 됨 — 2026-09-11 논의 참고). 절차:
  1. `git pull`로 최신 코드 받기 (`backtest/cache_warmup.py`, `stage1_signal_analysis.py`, `stage4_signal_analysis.py`, `warning_tag_gate_analysis.py` 전부 2026-09-18부로 `--start`/`--end` CLI 인자 지원 — `config/params.yaml`을 직접 고쳤다 되돌릴 필요 없음)
  2. 데이터 품질 소규모 파일럿 먼저: 분기 1~2개만 캐시 웜업 후 결측률 확인(2014~2015년 DART 표준 재무제표 공시가 상대적으로 덜 정착된 시기라 결측이 많을 수 있음, 2026-09-15 논의 참고)
  3. `python3 backtest/cache_warmup.py --start 2014 --end 2018` (DART 캐시 예열, 전체 실행 전 사용자 확인)
  4. `caffeinate -i python3 backtest/run_backtest.py --start 2014 --end 2018` (기준 백테스트)
  5. 저비용 신호 분석 재실행: `stage1_signal_analysis.py --start 2014 --end 2018`, `stage4_signal_analysis.py --start 2014 --end 2018`, `warning_tag_gate_analysis.py --start 2014 --end 2018` — 2019~2025와 같은 "약한 신호 앙상블" 패턴이 재현되는지, 아니면 이 국면에선 다른 팩터가 힘을 받는지 비교
  6. `config/params.yaml`은 두 기기에서 동일하게 유지(기간만 CLI로 다르게) — 결과는 이 문서에 날짜와 함께 기록
  - **진행 상황 (2026-09-20, 상세는 [decisions_log.md](./decisions_log.md) 2026-09-20 항목)**:
    - 별도 키 발급이 불가(아이디당 1개)해 맥과 같은 DART 키를 공유 중 — 한 기기에서만 DART를 호출하고 합산 20,000건/일을 넘기지 않도록 조율 필요(호출 카운터는 기기별 로컬 파일).
    - 데이터 경계: 2015년은 사업보고서만 있고 분기 보고서는 전부 없음 → 분기 재무 기반 판정은 2016년부터 가능, 2016년 4개 분기는 종목 0개(현금). 그래서 실질 검증 구간은 2017-03 이후.
    - 3단계(웜업)·4단계(기준 백테스트) 완료: 2016~2018 전략 누적 -10.1% vs KOSPI +2.0%(운용 7개 구간만 -10.1% vs -5.5%), MDD -21.2% vs -15.5%, Sharpe -0.27 — 실질 표본 7개 분기라 과적합 여부는 판단 불가.
    - 5단계(신호 분석 3종) 완료 (2026-09-21): KRX 접속 제한(2026-09-20 14시 무렵, 병렬 실행이 원인으로 추정)이 풀린 뒤 **순차 재실행** — Stage1 → Stage4 → 게이트 모두 2016~2025 전체 39분기, 오류 건너뜀 0(안전장치는 발동하지 않았고 실제 차단 상황에서의 동작은 여전히 미검증). 2019~2025 구간만 뽑으면 기존 기록이 정확히 재현됨(Stage1 표본 3,335개, Stage4 표본 1,234개와 팩터별 IC, 게이트 5개 태그의 차이·p값). `analysis/signal_period_breakdown.py`로 2016~2018(OOS)/2019~2021/2022~2025/전체를 재집계한 결과:
      - **Stage1**: 두 신호 모두 어느 구간에서도 유의하지 않음(전체 분기평균 IC t=0.29/0.92) — 가중치 0.5/0.5 결론 유지.
      - **Stage4는 국면 의존적**: `per_inv_z`는 2016~2018에서 가장 강한 팩터(IC +0.088, t=2.52)였으나 2019~2025에서는 가장 나쁨(-0.051). `pbr_inv_z`는 2016~2021에서 양(+0.09/+0.08)이나 2022~2025에서 소멸. `bps_z`는 세 구간 모두 신호 없음(-0.006/-0.053/+0.006). → `per_weight=0.0` 결정의 근거("PER의 IC가 최악")는 2019~2025 국면에 한정되어 약해졌으나 뒤집히지는 않음(OOS는 8개 분기뿐, 3팩터×4구간 다중비교).
      - **warning_tags 5종 모두 게이트 전환 근거 없음**: `is_cost_cutting_warning`은 전체에서 경고 종목이 오히려 유의하게 더 좋고(+2.23%p, p=0.008), `[이익질주의]`는 OOS에서 효과 0(-0.08%p)·전체 -2.59%p(p=0.11)로 약해졌으며(위 TODO 우선순위 하향), `is_pbr_value_trap`은 2016~2021 우위·2022~2025 열위로 국면에 따라 뒤집힘. p값은 같은 분기 종목이 독립이 아니고 다중비교(5태그×4구간)라 낙관적으로 읽어야 함.
    - 명령어 참고: 데스크탑(Windows)에서는 `python3`/`caffeinate` 대신 `quant_env\Scripts\python.exe backtest\...`를 쓴다(절전 방지는 전원 설정으로 별도 처리).
    - 캐시 이전: 맥의 `data/cache`를 옮겨 2019~2025를 재현 — 기존 기록과 완전 일치(누적 54.3%·MDD -25.1%·Sharpe 0.32·승률 37.0%). 이전 파일의 수정 시각이 리셋되어 `.empty`/시세/유니버스 캐시가 2026-10-20 무렵 일괄 만료되므로 그 전에 분석을 끝내거나 수정 시각을 갱신할 것.
    - **파이프라인 전체 효과는 미확립** (2026-09-21): 분기 초과수익(전략−KOSPI) 평균이 2019~2025 -0.98%p(t=-0.68, 27분기)·2016~2018 운용 7분기 -0.41%p(t=-0.13)로 모두 0과 통계적으로 구분되지 않고, 회귀 알파는 +0.05%p/-0.42%p, 베타는 0.66/0.98. 2019~2025의 낮은 MDD(-25.1% vs -32.3%)와 양의 Sharpe(0.32)는 종목 선정 효과라기보다 낮은 베타(0.66)와 구분되지 않으며, 2016~2018에서는 그 방어 효과도 없었음(MDD -21.2% vs -15.5%, 베타 0.98). 다만 표본이 작아(27/7분기, 분기 초과수익 표준편차 7.5~8.3%p) 효과가 없다고 단정할 수도 없음 — 공통 설계 원칙 9번의 단서와 아래 신규 TODO 참고.
- [ ] 파이프라인 효과의 직접 검증 (2026-09-21 제안, 미착수) — 지금까지의 분석은 "개별 신호가 다음 분기 수익률과 상관이 있나"였고 파이프라인 전체 효과는 직접 재지 못했음. (a) 단계별 통과 vs 탈락 종목의 다음 분기 수익률 비교: 각 stage의 입력 후보군에서 통과/탈락 그룹을 비교(탈락 종목이 보존되므로 `warning_tag_gate_analysis.py`와 같은 방식으로 저비용 가능). (b) 무작위 대조군: 같은 분기 Stage1(또는 Stage2) 통과 종목에서 최종 종목 수와 같은 개수를 무작위로 뽑은 포트폴리오를 다수 생성해 실제 파이프라인 성과가 그 분포의 어디에 놓이는지 확인 — 종목 선정 효과와 저베타 종목군 효과를 분리. KRX 조회가 있는 스크립트는 순차 실행 원칙 준수.
...
| 2026-09-21 | 데스크탑 out-of-sample 검증 완료 — KRX 제한 해제 후 신호 분석 3종(Stage1/Stage4/warning_tags)을 순차로 2016~2025 전체 재실행(39분기, 오류 0)하고 구간별 재집계. 2019~2025 부분은 기존 기록과 정확히 재현. Stage1은 신호 없음 유지, Stage4는 국면 의존(PER·PBR 부호가 2016~2021 vs 2022~2025에서 반전, bps_z는 무신호), warning_tags는 게이트 전환 근거 없음(`[이익질주의]`도 OOS 효과 0이라 우선순위 하향). 파이프라인 전체 초과수익이 통계적으로 0과 구분되지 않음(-0.98%p/-0.41%p, 알파≈0, 낮은 MDD는 낮은 베타로 설명 가능)을 확인해 공통 설계 원칙 9번에 단서를 추가하고 효과 직접 검증 TODO 신설 |
| 2026-09-20 | 데스크탑 2016~2018 out-of-sample 검증 진행 — DART 분기 재무가 2016년부터라 범위를 2014~2018에서 2016~2018로 축소(2016년 4개 분기는 종목 0개), 기준 백테스트는 실질 7개 분기에서 전략 -10.1% vs KOSPI -5.5%로 판단 불가. 맥 캐시 이전 후 2019~2025 재현 결과가 기존 기록과 완전 일치(54.3%/-25.1%/0.32/37.0%)함을 확인. DART 일일 한도 9,500→19,000, 업종 목록 404 폴백 추가. KRX가 IP를 1일 제한(병렬 실행이 원인으로 추정)해 신호 분석은 Stage1 유효분만 확보, `analysis_guard.py`(연속 실패 중단·중간 저장)와 `signal_period_breakdown.py` 신설. TODO 진행 상황 갱신 |
| 2026-09-18 (2) | `cache_warmup.py`/`stage1_signal_analysis.py`/`stage4_signal_analysis.py`/`warning_tag_gate_analysis.py`에 `--start`/`--end` CLI 인자 추가(`run_backtest.py`와 동일 패턴) — 데스크탑에서 2014~2018 out-of-sample 검증을 `config/params.yaml` 수정 없이 바로 시작할 수 있도록. TODO에 데스크탑 검증 절차 상세 기록 |
| 2026-09-18 | 기존 `warning_tags` 5종의 게이트 전환 가치 검증 — `backtest/warning_tag_gate_analysis.py` 신설, flagged vs unflagged forward return 비교. `is_cost_cutting_warning`은 오히려 유의하게 역방향(+2.00%p, p=0.031), `is_pbr_value_trap`·`[ICR미달]`·`[과다부채]`도 효과 없거나 역방향 — 4개 태그는 게이트 전환 시 해가 됨을 확인. `[이익질주의]`만 예상 방향(-3.36%p, p=0.068, 근접 유의)으로 유일한 유망 후보, 실제 게이트 전환 A/B는 TODO로 이관 |
| 2026-09-17 (4) | "정교화가 왜 역효과인가" 근본원인 분석 — `backtest/stage4_signal_analysis.py` 신설, Stage4 3개 팩터(pbr_inv_z/bps_z/per_inv_z) 개별 IC 측정(1234표본). `per_inv_z`가 가장 나쁜 IC(-0.0511)로 확인돼 희석 가설 입증, 실전 가중치 0.8인 `bps_z`조차 단독 IC가 음수임을 발견 — "약한 개별 신호들의 순차 필터 앙상블" 구조로 결론, 공통 설계 원칙 9번으로 문서화 |
| 2026-09-17 (3) | Stage4 `per` composite score 반영 A/B 백테스트 완료 — `per_weight` 파라미터 신설(기본 0.0), PBR 0.1/PER 0.1/BPS 0.8로 검증한 결과 누적수익률(22.4% vs 54.3%)·MDD(-29.7% vs -25.1%)·Sharpe(0.15 vs 0.32) 전부 열세로 확인. annual·avg_4q에 이어 "기존 설계에 차원 추가" 시도 3연속 실패 — 기본값 `per_weight: 0.0` 유지 확정 |
| 2026-09-17 (2) | Stage1 하드컷→penalty 전환(C안) 저비용 사전검증 — `debugging/stage1_bypass_diagnostic.py` 신설, 방산 5종목x3개 분기를 실제 Stage1 통과 후보군(344~395개)에 끼워 Stage2~5를 돌려본 결과 15건 중 2건(13%)만 최종 통과, 나머지는 Stage4 밸류에이션에서 정상적으로 걸러짐. C안 기대 효과가 가설보다 작다고 판단해 우선순위 하향 |
| 2026-09-17 | Stage1 z_return/z_volume 가중치(w1/w2) forward-return IC 분석 완료 — `backtest/stage1_signal_analysis.py` 신설, 27분기·3335표본으로 두 신호 모두 forward 섹터 수익률과 통계적 유의성 없음 확인(풀링/분기평균/분위스프레드 전부 비유의, 부호도 불안정). 가중치 0.5/0.5 기본값 유지 확정 — Stage1은 강한 알파 신호가 아니라 1차 필터+계산비용 관문 역할로 충분하다는 결론 |
| 2026-09-14 | `global.ttm_denominator: "avg_4q"` vs `"latest_snapshot"` A/B 백테스트 완료 — 27개 분기 전체 실행 결과 avg_4q가 누적수익률(29.9% vs 54.3%)·MDD(-26.4% vs -25.1%)·Sharpe(0.19 vs 0.32)·승률(29.6% vs 37.0%) 전부에서 열세로 확인. 평균 보유 종목 수도 소폭 증가(12.0→12.9개) — annual 검증과 같은 패턴(분모 스무딩이 위험 신호 지연 인식으로 이어짐). 기본값 `"latest_snapshot"` 최종 확정 |
