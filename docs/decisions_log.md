...
### [결정] Stage1 하드컷→penalty 전환(C안) 저비용 사전검증 — 기대 효과가 가설보다 작음을 확인, 우선순위 하향
- **배경**: 남은 TODO 중 "기대 효과가 클 것 같은 것"을 추천해달라는 요청에, 2026-09-08에 정량적으로 확인된 유일한 대형 손실 원인(방산 등 극단 모멘텀 섹터의 섹터 단위 배제)을 겨냥하는 C안을 추천. 다만 구현 비용(DART 예산 재폭발 위험, 설계 복잡도)이 큰 항목이라, 사용자와 합의해 큰 설계에 들어가기 전에 "정말 효과가 있을지"부터 싸게 검증하기로 함.
- **1차 시도(방법론 결함, 즉시 재설계)**: 방산 5종목(079550 LIG넥스원, 103140 풍산, 012450 한화에어로스페이스, 272210 한화시스템, 064350 현대로템)만 단독으로 Stage2~5에 통과시켜봤더니, Stage3/4의 Z-score·percentile 컷오프가 5개짜리 소표본 기준으로 계산되어 왜곡됨을 발견(Stage5가 과거 N=1 문제로 pool 평가로 전환했던 것과 동일한 통계적 함정). 즉시 재설계.
- **2차 시도(올바른 방법론)**: `debugging/stage1_bypass_diagnostic.py` 신설 — 해당 base_date의 **실제 Stage1 통과 후보군 전체**(344~395개)를 정식으로 구한 뒤, 방산 5종목을 그 안에 강제로 끼워 넣어(이미 통과한 건 중복 추가 안 함) Stage2~5를 실제 파이프라인과 동일한 `_accumulate_results`/`_filter_passed`로 순차 실행. 대상 분기는 2026-09-08 진단에서 언더퍼폼 격차가 가장 컸던 2024-12-30/2025-03-31/2025-06-30 세 분기.
- **결과**: 15개 종목x분기 조합 중 **2건(13%)만 최종 통과**(012450 @ 2025-03-31, 064350 @ 2025-06-30). 나머지 13건은 대부분 Stage4(밸류에이션)에서 `composite_score_below_cutoff`로 탈락 — 이미 급등한 뒤라 PBR이 비싸져서 밸류에이션 필터가 정상적으로 작동해 걸러낸 것으로 해석(전략이 고장난 게 아니라 의도대로 작동한 결과).
- **결론**: "Stage1이 안 막았다면 방산 랠리를 대부분 놓치지 않았을 것"이라는 최초 가설과 달리, 개별 기업 단위로 봤을 때 회수 가능한 몫은 훨씬 작음(15개 중 2개). C안의 구현 비용·리스크는 그대로인데 기대 효과는 작아진 셈이라 **우선순위를 하향**하기로 사용자와 합의 — 완전히 폐기하지는 않되, Stage4 `per` 반영 여부·Stage3/4 컷오프 튜닝처럼 더 싸고 확실한 항목을 먼저 진행.
- **한계**: n=15(방산 5종목 x 3분기)로 표본이 작음 — 2021Q4~2025Q3 전체 기간이나 다른 극단 모멘텀 섹터까지 포함한 전면 재검증은 아님. 우선순위 판단용 저비용 신호로만 사용.
- → `debugging/stage1_bypass_diagnostic.py`(신규) 반영 완료. `screening_criteria.md`(C안 TODO 우선순위 하향 표시) 반영 완료.

### [결정] Stage4 `per` composite score 반영 A/B 백테스트 — 열세로 확인, 미반영(0.0) 유지 확정
- **배경**: C안 우선순위를 낮춘 뒤, 더 싸고 확실한 항목으로 Stage4 `per` 반영 여부 TODO에 착수. `per`는 2026-09-01부터 pykrx 응답에서 값만 보존하고 스코어링엔 미반영 상태였음.
- **구현**: `stages/stage4_valuation.py`에 `per_weight` 파라미터 신설(기본 0.0 — 0이면 `per_inv_z` 계산 자체를 스킵해 기존 동작과 완전히 동일, weight dict에 키가 아예 안 들어가므로 0×NaN 오염 위험도 없음). 0보다 크면 PBR과 동일한 방식(1/per, 자본잠식 등 0 이하는 NaN 구제)으로 z-score화해 합산에 포함. `config/params.yaml`에 `per_weight: 0.0` 추가.
- **검증**: 005930/000660/019180 소표본으로 `per_weight=0.0`(기존과 동일) vs `0.1`(값 반영) 두 케이스가 정상적으로 다른 `stage4_score`를 내는지 먼저 확인 후, PBR 0.1/PER 0.1/BPS 0.8(기존 PBR 0.2 중 절반을 PER로 나눔, "성장주 프리미엄"인 BPS 0.8은 유지)로 27개 분기 전체 백테스트 실행.
- **결과**:

| 지표 | per_weight=0.0(기본값) | per_weight=0.1 |
|---|---|---|
| 누적수익률 | 54.3% | **22.4%** |
| CAGR | 6.6% | 3.0% |
| MDD | -25.1% | **-29.7%**(더 나쁨) |
| Sharpe | 0.32 | 0.15 |
| 벤치마크 대비 승률 | 37.0% | 29.6% |
| 평균 보유 종목 수 | 12.0 | 16.4 |

- **메타 관찰**: `profitability_basis: "annual"`(2026-09-12), `ttm_denominator: "avg_4q"`(2026-09-14)에 이어 이번 `per` 반영까지, "기존 설계에 차원/정교함을 추가"하는 시도가 **3연속으로 성과를 악화**시켰다. 세 경우 모두 공통적으로 평균 통과 종목 수가 늘어나는 패턴을 보임(annual 12.0→14.9, avg_4q 12.0→12.9, per 12.0→16.4) — 컴포짓 스코어에 차원을 추가할수록 상위 N% percentile 컷오프의 실질 문턱이 느슨해져, 원래라면 걸러졌을 종목이 통과하는 방향으로 작동하는 것으로 보인다. 이 백테스트 기간(2019~2025)에 대해서는 현재의 단순한 설계(TTM 단일시점, latest_snapshot 분모, PBR/BPS만 쓰는 밸류에이션)가 우연이 아니라 실제로 국소최적점에 가깝다는 정황 — 향후 유사한 "정교화" 제안은 이 패턴을 감안해 더 회의적으로 접근할 필요.
- **최종 결정**: `per_weight: 0.0` 기본값 유지 확정. 코드는 보존(향후 다른 가중치 조합으로 재시도 가능).
- → `stages/stage4_valuation.py`(`per_weight` 신규), `config/params.yaml`(`per_weight: 0.0`) 반영 완료. `screening_criteria.md`(TODO 종료 처리) 반영 완료.

### [결정] "정교화가 왜 계속 역효과인가" 근본 원인 분석 — Stage4 팩터별 IC로 "희석 가설" 확인
- **배경**: annual → avg_4q → per, 세 번 연속 "기존 설계에 차원 추가" 시도가 성과를 악화시킨 패턴을 사용자가 지적("시장은 원채 복잡도가 큰 모델이라 오히려 간단한 로직이 더 잘 먹히는 것 같다"). 다음 튜닝으로 바로 넘어가는 대신, 이 패턴 자체를 먼저 분석하기로 합의.
- **가설**: 새로 추가한 팩터(PER) 자체의 예측력이 약해서, 합산 시 기존 좋은 신호(PBR)를 희석시켰을 것.
- **검증**: `backtest/stage4_signal_analysis.py` 신설(Stage1 IC 분석과 동일한 방법론 — 풀링 Spearman + 분기별 Fama-MacBeth). 27개 분기 각각에서 **실제 Stage3 통과 후보군**(Stage4의 진짜 입력 모집단, `pipeline.run(stop_after=Stage.STAGE3)`으로 재현)을 구하고, `stages/stage4_valuation.py`의 팩터 계산 로직을 그대로 복제해 `pbr_inv_z`/`bps_z`/`per_inv_z` 각각을 종목별 forward return과 상관분석. 1234개 표본 확보(2시간 소요, 이번 주 여러 백테스트로 캐시가 예열돼 있어 예상보다 빠름 — 도중 KRX 세션 만료 1회 발생했으나 pykrx 자체 세션 자동 갱신이 정상 작동해 문제 없이 계속됨, 참고로 남겨둠).
- **결과**:

| 팩터 | 풀링 IC | 분기평균 IC (t-stat) |
|---|---|---|
| `pbr_inv_z`(기존) | 0.0211 | 0.0265 (t=0.89) |
| `bps_z`(기존, 가중치 0.8) | -0.0330 | -0.0200 (t=-0.70) |
| `per_inv_z`(신규) | -0.0149 | **-0.0511 (t=-1.28)** |

- **결론 1 (희석 가설 확인)**: `per_inv_z`가 세 팩터 중 가장 나쁜(가장 음의) 개별 IC를 보임 — 단순 노이즈 추가가 아니라 방향 자체가 살짝 반대(저PER 종목이 오히려 소폭 부진)라, 기존에 유일하게 양의 방향이던 `pbr_inv_z`를 합산 과정에서 상쇄·희석시킨 것으로 해석. per_weight A/B 백테스트의 성과 악화를 팩터 수준에서 직접 설명하는 근거 확보.
- **결론 2 (더 큰 발견)**: 실전 가중치 0.8로 가장 크게 반영되는 `bps_z`조차 단독 IC가 음수(-0.02)임에도, 그 가중치를 그대로 쓰는 전체 파이프라인은 27분기 백테스트에서 누적 54.3%·양의 Sharpe라는 검증된 성과를 냄. Stage1의 두 신호(2026-09-17 앞선 분석, 둘 다 유의하지 않음)와 이번 Stage4의 세 팩터(전부 유의하지 않거나 약함) 모두 **개별 선형 예측력은 약하거나 없는데, Stage1~5가 순차적으로 후보군을 좁혀가는 필터링 조합 자체가 실제 성과를 만들어낸다**는 그림이 이제 두 단계(Stage1, Stage4)에서 일관되게 확인됨 — 사용자가 제시한 "복잡계 시장에서는 단순한 개별 로직들의 조합이 더 잘 먹힌다"는 해석과 부합. 이는 "각 팩터를 더 정교하게 다듬으면 더 나아질 것"이라는 직관이 이 프로젝트의 실제 데이터 위에서는 반복적으로 틀렸던 이유에 대한 근거 있는 설명이 됨.
- **시사점**: 향후 "정교화" 제안(가중치 세분화, 신규 팩터 추가, 비선형 결합 등)은 이 패턴을 감안해 개별 팩터의 IC부터 저비용으로 먼저 확인하고 나서 전체 백테스트에 투자하는 순서를 기본으로 삼는다(Stage1/Stage4에서 이미 두 번 검증된 방법론이므로 재사용 가능).
- → `backtest/stage4_signal_analysis.py`(신규) 반영 완료. 코드/설정 변경 없음(분석 결과만 기록).

### [결정] 기존 warning_tags(5종)를 실제 배제 게이트로 전환할 가치가 있는지 검증 — 4개는 오히려 해로움, 1개만 유망
- **배경**: "기존 팩터를 정교화하는 대신, 거친 게이트를 하나 더 추가하는 게 낫지 않겠냐"는 논의(순차 필터=약한 신호 앙상블이라는 방금 발견과 부합). 사용자가 "이미 구현된 걸 최대한 활용하자"고 확정 — 새 신호를 발굴하는 대신, 이미 계산되지만 판정에 미반영인 warning_tags(`is_cost_cutting_warning`, `is_pbr_value_trap`, `[ICR미달]`, `[이익질주의]`, `[과다부채]`)를 게이트 후보로 검토.
- **방법**: `backtest/warning_tag_gate_analysis.py` 신설. 27개 분기 각각에서 Stage1→2→3→4→5를 실제 파이프라인과 동일하게(`_accumulate_results`/`_filter_passed` 재사용) 순차 실행하며, 각 태그가 계산되는 stage의 **입력 후보군 전체**(통과/탈락 무관)에서 태그 계산 → 해당 종목의 forward return을 조회. flagged 그룹과 unflagged 그룹의 평균 forward return을 Welch's t-test로 비교(등분산 가정 안 함). 이번 주 여러 백테스트로 Stage1~4 DART 캐시가 예열돼 있어 비교적 빠르게(중간에 세션 만료 2회, pykrx 자체 재로그인으로 정상 처리) 완주.
- **결과**:

| 태그 | flagged 평균 | unflagged 평균 | 차이 | t / p |
|---|---|---|---|---|
| `is_cost_cutting_warning`(매출역성장 -5%↓) | 3.83%(n=831) | 1.83%(n=2560) | **+2.00%p** | t=2.16, **p=0.031(유의)** |
| `is_pbr_value_trap`(저PBR+저ROE) | 3.48%(n=152) | 3.86%(n=1082) | -0.38%p | t=-0.17, p=0.87 |
| `[ICR미달]` | 4.24%(n=49) | 2.29%(n=361) | +1.95%p | t=0.73, p=0.47 |
| `[이익질주의]`(OCF<순이익) | 0.54%(n=168) | 3.90%(n=242) | **-3.36%p** | t=-1.83, p=0.068(근접 유의) |
| `[과다부채]` | 2.82%(n=321) | 1.45%(n=89) | +1.37%p | t=0.64, p=0.52 |

- **해석**: `is_cost_cutting_warning`은 "경고가 붙은 종목이 오히려 통계적으로 유의하게 더 좋은 성과"를 내는, 원래 가설과 정반대 결과 — 매출 역성장을 겪은 뒤 반등하는 턴어라운드 효과이거나 이미 악재가 주가에 반영된 뒤라는 해석이 가능. `[ICR미달]`·`[과다부채]`도 방향이 뒤집혀 있음(통계적으로 유의하진 않지만, 최소한 게이트로 쓸 근거는 없음). `is_pbr_value_trap`은 완전히 무의미(효과 없음). 5개 중 4개가 "직관적으로 위험해 보이지만 데이터로는 위험하지 않거나 오히려 반대"인 셈 — Stage1/Stage4 IC 분석에 이어 세 번째로, "그럴듯한 직관"이 이 데이터에서 검증 없이는 신뢰할 수 없다는 걸 재확인.
...
### [결정] 데스크탑 out-of-sample 검증 범위를 2014~2018 → 2016~2018로 축소 — DART 분기 재무 데이터가 2016년부터 존재
- **배경**: 2026-09-18에 세운 계획(2014~2018)을 데스크탑(Windows, Ryzen 5 7600)에서 실행 시작. 맥과 같은 DART 키를 공유(아이디당 키 1개라 분리 불가)하므로, 호출을 쓰기 전에 데이터가 실제로 있는 범위부터 확인.
- **발견**: OpenDartReader `finstate_all`은 "2015년 이후 데이터 제공"이라 안내하지만, 실제 캐시는 2015년에 사업보고서(11011)만 있음(711종목 중 523건, 74%). 1분기·반기·3분기 보고서(11013/11012/11014)는 711종목 전부 비어 있음. 2016년부터 4종 모두 존재(각 약 515~540건). 즉 분기 재무(TTM·전년동기 비교·판관비율 등)는 사실상 2016년 1분기 보고서부터 존재하며, 2014~2015년 분기는 판정에 쓸 수 없고 웜업 호출량만 낭비(맥 캐시에 2014년 데이터 0건, 2013년은 조회해 보지 않음; 빈 응답도 한도에 포함).
- **해결**: `--start 2016 --end 2018`로 축소. 웜업은 시작연도-1(2015)부터 조회 — 711종목 × 4개 연도 × 4종 = 11,376건, 83.6분(Stage1 리졸버와 중간의 짧은 절전 구간 포함 — 호출당 약 0.44초로 네트워크 대기가 대부분), 한도 초과(`020`) 응답 0건. 백테스트 12분기는 52.9분(DART 6,589건 — 웜업하지 않은 개별재무제표(OFS) 폴백 조회 등으로 추정, 구성은 미확인).
- **결과(2016~2018 기준 백테스트, 12개 리밸런싱일·11개 구간)**: 2016년 4개 분기는 Stage2/3 이력 부족으로 종목 0개(현금 처리), 실제 운용은 2017-03-31 진입 이후 7개 구간(종목 8~20개). 전체 11개 구간: 전략 누적 -10.1% vs KOSPI +2.0%, MDD -21.2% vs -15.5%, Sharpe -0.27 vs -0.05, 승률 45.5%. 운용 7개 구간만: -10.1% vs -5.5%, Sharpe -0.27 vs -0.30, 승률 42.9%. (Sharpe는 분기 4회 연환산 — 2019~2025의 0.32가 이 방식으로 재현됨. 문서 내 3.35는 일 252회 연환산 표기.) 실질 표본이 7개 분기뿐이라(2019~2025는 27개) 이 결과만으로 "2019~2025 결론이 과적합이다/아니다" 어느 쪽도 판단할 수 없음.
- **참고 — 한도 초과 응답 캐싱 위험**: `finstate_all`은 응답에 `list`가 없으면(`020` 포함) 오류를 출력만 하고 빈 DataFrame을 반환 → `loader.get_financial_statements`가 `.empty` 마커(30일 유효)로 캐싱해 "가짜 없음"이 남을 수 있음. 공유 키에서 실제 위험이지만 이번엔 코드 보강 없이 진행(사용자 결정) — 대신 로그에서 `'status': '020'` 부재를 확인함(0건). 미해결 위험으로 유지.

### [결정] DART 일일 호출 한도 9,500 → 19,000, 업종 목록 조회 404 폴백 추가
- **배경(한도)**: OpenDART 공식 개발가이드 에러코드 020 — "일반적으로는 20,000건 이상의 요청에 대하여 이 에러 메시지가 발생". `endpoints.json`에 `daily_limit` 항목이 없어 `loader.py`의 기본값 9,500이 쓰이고 있었음.
- **해결(한도)**: 20,000건 기준 5% 마진으로 19,000. `endpoints.json`에 `daily_limit` 명시, `loader.py` 기본값도 동일, `cache_warmup.py`의 하드코딩 "10,000건" 문구를 실제 값 출력으로 수정. 한도 초과 시 기존처럼 `RuntimeError`로 중단(조용히 넘어가지 않음).
- **한계**: 호출 카운터(`dart_call_state.json`)는 기기별 로컬 파일이라 맥과 데스크탑이 같은 키를 쓰는 동안 서로의 호출량을 모름 — 합산이 20,000건을 넘지 않게 사람이 조율해야 함(한 기기에서만 DART 호출 작업).
- **배경(404)**: 데스크탑 첫 웜업이 Stage1에서 12개 분기 전부 `HTTP 404`로 실패. 원인은 `fdr.StockListing('KRX-DESC')`가 KRX가 알려준 최신 영업일 이름의 CSV를 GitHub 미러(`FinanceData/fdr_krx_data_cache`)에서 읽는데 그 날짜 파일이 아직 올라오지 않은 것(과거 기준일도 같은 목록을 호출해 전부 실패). 맥은 캐시된 유니버스 덕에 이 경로를 안 탔음.
- **해결(404)**: `loader._get_sector_listing()` 신설 — 404일 때만 오늘부터 하루씩 과거 날짜 파일(최대 14일)로 폴백, 인스턴스 내 재사용. 업종은 어차피 base_date와 무관한 "현재 분류"(알려진 한계)라 며칠 전 분류를 써도 결과에 실질적 영향 없음. 2016-03-31/2019-03-29로 Stage1 시험 통과.
- → `config/endpoints.json`, `data/loader.py`, `backtest/cache_warmup.py` 반영 완료 (커밋 976d6c1).

### [결정] 맥 캐시를 데스크탑으로 이전하고 동등성 검증 — 2019~2025 재현 결과가 문서 값과 완전 일치
- **배경**: 2019~2023 재무 캐시를 데스크탑에서 다시 받으면 27,000건 이상(한도 기준 이틀, API 대기 3시간+). 맥의 `data/cache`(39,085개, 1.1GB, git 제외)를 압축해 옮김.
- **검토**: 이름이 겹치는 8,015개 중 6,214개가 바이트 단위로 달랐으나 원인은 줄바꿈(CRLF/LF)뿐(dart csv 표본 300/300; 비-dart 5개 중 실제 차이는 `universe_20190329.csv` 1개 — 791종목 중 1종목의 업종 라벨). `.csv`↔`.empty` 충돌 0건. 복사 과정에서 수정 시각이 전부 복사 시각으로 리셋되어 `.empty`/시세/유니버스 캐시는 2026-10-20 무렵 일괄 만료로 판정됨(dart csv는 유효기간 3,650일이라 무관).
- **가짜 없음 점검**: 맥 캐시에서 2023년의 "일부 보고서만 없음" 비율이 7.0%로 이웃 연도(약 3%)의 두 배라 재조회 — 2023년 의심 119건 + 대조군 2022년 51건 전수를 DART에 직접 조회한 결과 170건 전부 실제 없음(`013`), `020` 없음. 오염 없음(다른 연도의 마커까지 전수 검증한 것은 아님). 캐시는 수정하지 않는 읽기 전용 진단(호출 340건은 로컬 카운터에 미반영).
- **이전**: 신규 31,070개 추가(기존 파일 덮어쓰기 없음, `dart_call_state.json` 제외 — 이 기기의 카운터 보호), `universe_20190329.csv`만 맥 버전으로 교체(상장폐지 종목의 업종 라벨 보존, 원본 백업). 복사 목록은 사전에 저장(롤백 가능). 이전 후 루트의 임시 압축 해제 폴더는 삭제.
- **검증**: 데스크탑에서 2019~2025 기준 백테스트 재실행 → 누적 54.3%·MDD -25.1%·Sharpe 0.32·승률 37.0%·평균 12.0종목이 기존 기록과 완전 일치, DART 호출 0건. 캐시가 채워진 상태에서 27분기 54분(분기당 약 2분) — `stage1_signal_analysis.py` 주석의 맥 기준 "27분기 각 2~4시간"과 비교해 2~4배 빠름(맥의 캐시 상태 등 실행 조건은 미확인이라 참고치).

### [사고] KRX가 IP를 1일간 접속 제한 — 신호 분석 병렬 실행이 원인으로 추정, 안전장치 추가
- **경과**: 2016~2025 전체 신호 분석을 Stage1+Stage4 병렬로 실행(KRX 과다 조회 사고 기록이 있어 세 번째 스크립트만 지연시켰지만 병렬 2개도 과했음). 14시 무렵(Stage1 15번째 분기 시점)부터 KRX가 JSON 대신 "KRX Data Marketplace 이용 제한 안내" 에러 페이지를 반환 — "자동화 수단을 통한 비정상 대량 조회가 감지되어 해당 IP의 접속이 일시적으로 제한", 탐지일로부터 1일, 해제 후 재탐지 시 재적용 가능. 2026-09-17에 이은 두 번째 사고. 원인은 병렬 실행이 유력하나 확정은 아님(두 스크립트가 분기마다 Stage1 섹터 지표 조회를 각자 중복 수행하는 등 동시 조회량이 컸던 것으로 추정 — 오류 메시지에 섹터 지수 조회 `get_index_ohlcv_by_date`가 반복해서 나타남. 참고로 Stage4의 수익률 조회는 분기당 시장 전체 1회 호출이라 종목별 대량 조회가 아님).
- **피해**: Stage1 분석은 15번째 분기(2019-09-30)부터 나머지 25분기를 전부 오류로 건너뛰고도 정상 종료 — 출력된 IC(-0.0154/0.0169, n=1,633)는 부분 표본이라 무효. Stage4 분석은 원자료 저장 전에 중단해 결과 소실, 게이트 분석은 KRX 로그인 단계에서 종료.
- **근본 문제**: 세 스크립트가 분기 예외를 "건너뜀"으로만 처리하고 끝까지 돌아 차단 상태에서도 정상 종료처럼 보임 + 원자료를 루프 종료 후에만 저장.
- **해결**: `backtest/analysis_guard.py` 신설 — 분기 3연속 실패 시 즉시 중단(추가 요청이 차단을 연장/재적용시키지 않도록), 분기마다 원자료를 `.partial.csv`에 이어쓰기, 종료 시 건너뛴 분기가 있으면 경고 + 종료코드 2. 3개 스크립트에 적용(Stage4의 "밸류에이션 원자료 없음"은 KRX 장애 증상으로 보고 오류로 집계, Stage3 후보 없음 등 정상적인 빈 결과는 오류로 세지 않음). 가짜 모듈로 정상/1분기 실패/3연속 실패/실패 후 회복/빈 후보 시나리오를 오프라인 시험해 16개 전부 통과(**실제 KRX 동작은 미검증** — 차단 중이라 실행하지 못함).
- **운영 원칙**: KRX 호출 스크립트는 순차 실행(병렬 금지 — 개인 프로젝트라 시간 제약이 없어 속도보다 안전 우선, 사용자 방침), 단계마다 `건너뜀` 개수와 표본 분기 수를 확인. 접속 제한은 2026-09-21 14시 무렵 해제 예상.
- **Stage1 유효 부분 결과**(정상 수집된 12개 구간, 2016-03~2018-12, n=1,393): `return_z_score` 풀링 IC -0.0179·분기평균 -0.0005(t -0.01), `volume_z_score` 풀링 0.0011·분기평균 0.0050(t 0.13) — 두 신호 모두 유의성 없음(2019~2025 결론과 같은 방향).
- **다음**: KRX 제한 해제 후 순차로 Stage1 → Stage4 → warning_tags 게이트를 2016~2025 전체로 재실행하고, `analysis/signal_period_breakdown.py`(신규, 파일만 읽는 순수 조회)로 2016~2018(OOS)/2019~2021/2022~2025/전체를 나눠 재집계. 이 스크립트는 Stage1 결과로만 시험했고 Stage4/게이트 부분은 실제 데이터로 아직 시험하지 못함.
- → `backtest/analysis_guard.py`(신규), `backtest/stage1_signal_analysis.py`, `backtest/stage4_signal_analysis.py`, `backtest/warning_tag_gate_analysis.py`, `analysis/signal_period_breakdown.py`(신규) 반영 완료.

## 2026-09-21

### [결정] 신호 분석 3종을 2016~2025 전체로 순차 재실행 — 2019~2025는 정확히 재현, 구간별로 국면 의존성 확인
- **경과**: KRX 접속 제한이 풀린 것(JSON 응답)을 요청 1건으로 확인한 뒤 Stage1 → Stage4 → warning_tags 게이트를 **한 번에 하나씩 순차** 실행(2016~2025 전체, 각 39분기). 세 스크립트 모두 오류 건너뜀 0, KRX 재차단 없음 — 09-20 사고의 원인이 병렬 실행이라는 추정과 일관되지만 이것만으로 확정할 수는 없음. `analysis_guard`는 한 번도 발동하지 않았으므로 실제 차단 상황에서의 동작은 여전히 미검증(오프라인 시험만 통과). 참고: 정상 종료 후에도 `.partial.csv`가 `outputs/`에 남음(삭제하지 않는 설계, git 대상 아님).
- **재현 검증**: 2019~2025 구간만 추출하면 기존 기록과 일치 — Stage1 표본 3,335개(1,454+1,881), Stage4 표본 1,234개와 팩터별 IC(pbr_inv_z 0.0211/0.0265 t=0.89, bps_z -0.0330/-0.0200 t=-0.70, per_inv_z -0.0149/-0.0511 t=-1.28), 게이트 5개 태그의 flagged/unflagged 표본 수와 차이(cost_cutting +2.00%p p=0.031, pbr_trap -0.38%p p=0.87, [이익질주의] -3.36%p p=0.067(기존 기록 0.068은 반올림 차이)). 같은 조건에서 구간만 넓혔다는 뜻.
- **Stage1 결과** (분기평균 IC, t-stat): 2016~2018 return_z -0.0005(-0.01)/volume_z 0.0050(0.13), 2019~2021 0.0316(0.76)/0.0368(0.66), 2022~2025 -0.0066(-0.17)/0.0230(0.71), 전체(39분기, n=4,728) 0.0070(0.29)/0.0217(0.92). 어느 구간에서도 유의하지 않고 부호도 불안정 → 가중치 0.5/0.5 유지 결론이 out-of-sample에서도 유지.
- **Stage4 결과** (분기평균 IC, t-stat; 2016~2018은 8개 분기, 2019~2021 12, 2022~2025 15, 전체 35):

  | 구간 | pbr_inv_z | bps_z | per_inv_z |
  |---|---|---|---|
  | 2016~2018 | +0.092 (1.84) | -0.006 (-0.10) | **+0.088 (2.52)** |
  | 2019~2021 | +0.078 (2.06) | -0.053 (-1.04) | -0.024 (-0.37) |
  | 2022~2025 | -0.015 (-0.35) | +0.006 (0.19) | -0.073 (-1.41) |
  | 전체 | +0.042 (1.61) | -0.017 (-0.66) | -0.019 (-0.58) |

  `per_inv_z`가 국면에 따라 부호가 뒤집힘(2016~2018 최강 → 2019~2025 최악) — 09-17에 "PER이 가장 나쁜 IC라 희석시켰다"로 결론 낸 근거는 2019~2025 국면에 한정된 것. 그러나 `per_weight=0.0` 결정 자체는 A/B 백테스트 성과(누적 22.4% vs 54.3%)에 근거하므로 **변경하지 않음**(OOS 표본은 8개 분기, 3팩터×4구간 다중비교로 t=2.52 하나를 결정적 증거로 보기 어려움). `bps_z`(실전 가중치 0.8)는 세 구간 모두 신호가 없음.
- **warning_tags 결과** (flagged−unflagged 다음 분기 수익률 %p, 괄호는 p값):

  | 태그 | 2016~2018 | 2019~2021 | 2022~2025 | 전체 |
  |---|---|---|---|---|
  | cost_cutting | +2.55 (0.25) | +4.47 (0.002) | -0.21 (0.86) | +2.23 (0.008) |
  | pbr_trap | +9.69 (0.013) | +4.43 (0.27) | -3.26 (0.22) | +1.58 (0.41) |
  | ICR미달 | 표본 부족(3개) | +1.43 (0.74) | +2.88 (0.40) | +1.96 (0.44) |
  | 이익질주의 | -0.08 (0.98) | -2.50 (0.43) | -3.46 (0.12) | -2.59 (0.11) |
  | 과다부채 | +3.55 (0.31) | +4.75 (0.16) | -0.74 (0.79) | +1.97 (0.28) |

  **결정**: 5개 태그 모두 게이트 전환 근거 없음. `is_cost_cutting_warning`은 전체에서 유의하지만 방향이 반대(경고 종목이 더 좋음)라 배제 기준으로 쓰면 해가 됨. 유일한 후보였던 `[이익질주의]`는 OOS 효과가 사실상 0이라 효과가 2022~2025에 몰려 있음 → 27분기 게이트 A/B 백테스트 TODO는 **우선순위 하향**. `is_pbr_value_trap`은 2016~2021 우위·2022~2025 열위로 Stage4 PER·PBR과 같은 국면 반전 패턴. 주의: t-검정이 종목을 독립으로 취급(같은 분기 종목은 국면 공유)하고 5태그×4구간 다중비교라 p값은 낙관적, Stage5 태그는 표본이 작음(전체 522개, 2016~2018 ICR미달은 3개).
- **파이프라인 전체 효과 검토** (사용자의 "개별 효과는 미미해도 합치면 효과가 있다는 결론은 유지되나?" 질문 계기): 분기 초과수익(전략−KOSPI) 평균이 2019~2025 -0.98%p(t=-0.68, 27분기, 전략>KOSPI 10/27분기)·2016~2018 운용 7분기 -0.41%p(t=-0.13, 3/7)로 모두 0과 구분되지 않음. 회귀 알파 +0.05%p/-0.42%p, 베타 0.66/0.98. 2019~2025의 낮은 MDD(-25.1% vs -32.3%)·양의 Sharpe(0.32)는 낮은 베타(0.66)와 구분되지 않고, 2016~2018에서는 방어 효과도 없었음(MDD -21.2% vs -15.5%). 수익률(54.3% vs 94.6%)·Sharpe(0.32 vs 0.46)는 KOSPI보다 낮음. **결론**: "약한 신호들의 순차 필터 앙상블이 성과를 낸다"(공통 설계 원칙 9번)는 설명은 반증되지 않은 가설이며 검증된 결론이 아님. 다만 표본이 작아(분기 초과수익 표준편차 7.5~8.3%p) 효과가 없다고 단정할 수도 없음. IC 분석은 "신호가 수익률과 상관있나"를 재는 것이라 파이프라인(임계 기반 순차 필터) 효과를 직접 재지 못함.
- **다음(TODO 신설, 미착수)**: (a) 단계별 통과 vs 탈락 종목의 다음 분기 수익률 비교 — 탈락 종목이 보존되어 있어 게이트 분석과 같은 방식으로 저비용 가능, (b) 무작위 대조군 — 같은 분기 Stage1(또는 Stage2) 통과 종목에서 최종 종목 수만큼 무작위로 뽑은 포트폴리오를 다수 만들어 실제 성과의 위치 확인(종목 선정 효과와 저베타 종목군 효과 분리). KRX 조회가 있는 스크립트는 순차 실행.
- → 코드 변경 없음. `screening_criteria.md`(데스크탑 검증 TODO 완료 처리, 진행 상황·[이익질주의] 우선순위 하향·신규 TODO·공통 설계 원칙 9번 단서·변경 이력) 반영 완료.
